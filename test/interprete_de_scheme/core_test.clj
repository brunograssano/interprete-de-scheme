...
(deftest evaluar-if-test
  (testing "Casos verdaderos"
    ...
    (is (= '(7 (n 7)) (evaluar-if (list 'if (symbol "#t") 'n 8) '(n 7))))
    (is (= '(7 (n 7)) (evaluar-if (list 'if (symbol "#T") 'n 8) '(n 7))))
    (is (= (list (symbol "#<unspecified>") '(n 8)) (evaluar-if (list 'if (symbol "#T") '(set! n 8) 1) '(n 7))))
    (is (= '(2 (+ +)) (evaluar-if '(if 1 (+ 1 1) 8) '(+ +))))
    )
  (testing "Casos falsos"
    (is (= (list (symbol "#<unspecified>")  '(n 7))
           (evaluar-if (list 'if (symbol "#f") 2) '(n 7))))
    (is (= (list (symbol "#<unspecified>") '(n 7))
           (evaluar-if (list 'if (symbol "#f") 'n) '(n 7))))
    (is (= '(8 (n 7))
           (evaluar-if (list 'if (symbol "#f") 'n 8) '(n 7))))
    (is (= '(1 (n 7 y 1))
           (evaluar-if (list 'if (symbol "#f") 'n 'y) '(n 7 y 1))))
    (is (= (list (symbol "#<unspecified>") '(n 7 f 3))
           (evaluar-if (list 'if (symbol "#F") 'n '(define f 3)) '(n 7))))
    (is (= (list (symbol "#<unspecified>") '(n 8))
           (evaluar-if (list 'if (symbol "#f") 1 '(set! n 8)) '(n 7))))
    (is (= '(4 (+ +))
